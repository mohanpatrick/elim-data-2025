...
# CHANGED: Added tryCatch to stop execution on HTTP 429 errors.
#          Kept single-argument signature to stay compatible with map() call below.
get_mfl_draft <- function(league_id) {
  ...
  tryCatch(
    {
      conn <- mfl_connect(search_draft_year, league_id, user_agent = mfl_client,
    ...
    error = function(e) {
      if (grepl("429", conditionMessage(e))) {
        stop("HTTP 429 Too Many Requests: rate limit hit on league_id ", league_id, call. = FALSE)
      }
      stop(e)
    }
  )
...
# CHANGED: Removed possibly() wrapper so that HTTP 429 errors thrown inside
#          get_mfl_draft() can propagate and halt execution. The downstream
#          nrow(mfl_drafts) < 1 check still handles empty results from other errors.
mfl_drafts <- mfl_leagues |>
  mutate(drafts = map(league_id, get_mfl_draft)) |>
  unnest(drafts)
